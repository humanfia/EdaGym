    ...
    for event in events:
        if isinstance(event, ParticipantIncarnationStartedEvent):
            ...
            if (
                ...
                or payload.incarnation.workspace_identity_digest == prior.workspace_identity_digest
...
def replay(header: RunHeader, task: TaskSpec, events: Sequence[RunEvent]) -> RunState:
    ...
    for expected_sequence, event in enumerate(events):
        ...
        if (
            expected_sequence == 1
            and requires_incarnation
            and not isinstance(event, ParticipantIncarnationStartedEvent)
        ):
            raise InvalidTransition(
        ...
        if isinstance(event, ParticipantIncarnationStartedEvent):
            ...
            provider_requests[started_payload.request_id] = ProviderRequestState(
                ...
                security_evidence_artifact_id=(started_payload.security_evidence_artifact_id),
                ...
                observed_input_token_floor=(started_payload.observed_input_token_floor),
            ...
            provider_requests[response.request_id] = request_state.model_copy(
                update={
                    ...
                    "provider_reported_service_tier": (response.provider_reported_service_tier),
            ...
            matching_tools = tuple(
                ...
                if tool.tool_id == reservation.tool_id and tool.capability is reservation.capability
            )
            ...
            reservation_event = participant_tool_reservations.get(terminal.request_interaction_id)
            ...
            if isinstance(event, ParticipantToolSettledEvent):
                if (
                    ...
                    or event.payload.input_manifest_digest != reservation.input_manifest_digest
                ):
                    raise InvalidTransition(
                        "participant tool settlement differs from its reserved operation"
                    )
                if (
                    event.payload.elapsed_milliseconds > reservation.reserved_compute_milliseconds
            ...
            evaluator_admitted = job_states.get(license_fact.job_id) is JobStateKind.QUEUED
    ...
    if (
        ...
        raise InvalidTransition("campaign and preflight runs require a participant incarnation")
...
def _validate_provider_transcript(
    ...
    if (
        record.logical_id != provider_transcript_artifact_id(event.payload.request_id, role)
...
class TrialJournal:
    """Release-trial replay over the shared atomic journal storage."""
    ...
    def __init__(self, directory: Path, header: RunHeader, task: TaskSpec) -> None:
        ...
        self._storage = JournalStorage(directory, run_journal_anchor(header), RunCommit)

    @classmethod
    def create(cls, state_root: Path, header: RunHeader, task: TaskSpec) -> TrialJournal:
        if task.digest != header.binding.task.task_spec_digest:
            raise JournalError("task identity does not match the run header")
        _validate_task_binding(header, task)
        directory = JournalStorage.create(
            state_root,
            header.run_id.removeprefix("sha256:"),
            {"run.json": canonical_bytes(header) + b"\n"},
        )
        return cls.open(directory, task)

    @classmethod
    def open(cls, run_directory: Path, task: TaskSpec) -> TrialJournal:
        require_directory(run_directory)
        content = read_file(run_directory / "run.json", maximum_bytes=4 * 1024 * 1024)
        try:
            header = RunHeader.model_validate_json(content)
    ...
    def record(self) -> RunRecord:
        ...
        return RunRecord(
            header=self.header,
            commits=tuple(RunCommit.model_validate(record.model_dump()) for record in records),
        )
    ...
    def integrity_digest(self) -> str:
        return self.record().integrity_digest
    ...
    def events_committed_together(self, event_ids: Sequence[UUID]) -> bool:
        ...
        return any(
            required.issubset(event.event_id for event in record.events)
            for record in self.record().commits
        )
    ...
    def append_events(self, requested: Sequence[RunEvent]) -> RunState:
        ...
        def commit(
            ...
            state = self._prospective_state(events, events_to_append)
            return events_to_append, state
    ...
    def transact_events(self, factory: Callable[[RunState], Sequence[RunEvent]]) -> RunState:
        def commit(
            records: tuple[JournalCommit[RunEvent], ...],
        ) -> tuple[tuple[RunEvent, ...], RunState]:
            events = tuple(event for record in records for event in record.events)
            ...
            if {event.event_id for event in events}.intersection(requested_ids):
                raise EventConflict("transaction reused a committed event identifier")
            return requested, self._prospective_state(events, requested)

        return self._storage.transact(commit)

    def _prospective_state(
        self, events: tuple[RunEvent, ...], requested: tuple[RunEvent, ...]
    ) -> RunState:
        ...
        if replay(self.header, self.task, events).terminal_reason is not None:
            raise InvalidTransition("a terminated run cannot accept another event")
        return replay(self.header, self.task, (*events, *requested))
